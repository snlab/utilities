...
from mininet.node import Node, OVSKernelSwitch, OVSSwitch, RemoteController,Controller
...
class NetworkTopo( Topo ):
    ...
    def build( self, **_opts ):
        ...
        s1 = self.addSwitch('s1', cls=MultiSwitch)
        ...
        # subnet 10.1.2.0/24
        s2 = self.addSwitch('s2', cls=MultiSwitch)
        self.addLink(r2, s2, intfName1='r2-', params1={'ip': '10.1.2.10/24'})        
        s3 = self.addSwitch('s3', cls=MultiSwitch)
        ...
        # subnet 10.1.3.0/24
        s4 = self.addSwitch('s4', cls=MultiSwitch)
        ...
        # subnet 10.1.4.0/24
        s5 = self.addSwitch('s5', cls=MultiSwitch)
...
def run():
    "Test linux router"
    topo = NetworkTopo()
    net = Mininet( topo=topo, controller=None, build=False, autoSetMacs=True )  # no controller needed
